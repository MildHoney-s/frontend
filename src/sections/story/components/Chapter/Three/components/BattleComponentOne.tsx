...
export default function BattleComponentOne() {
  ...
  useLayoutEffect(() => {
    const ctx = gsap.context(() => {
      ...
      // =========================================
      const playFlash = () => {
        gsap.fromTo('.white-flash', { opacity: 0.8 }, { opacity: 0, duration: 0.3, ease: 'power2.out', overwrite: true });
      }
      ...
      const startMagicIdle = () => {
        ...
        idleTween
          .to('.magic-dr-Skull', { y: -15, duration: 3, repeat: -1, yoyo: true, ease: 'sine.inOut' }, 0)
          .to('.magic-dr-arm-l', { y: -10, rotation: 3, duration: 2.5, repeat: -1, yoyo: true, ease: 'sine.inOut' }, 0)
          .to('.magic-dr-arm-r', { y: -8, rotation: -3, duration: 2.8, repeat: -1, yoyo: true, ease: 'sine.inOut' }, 0);
      }
      ...
      const stopMagicIdle = () => {
        ...
        gsap.set(['.magic-dr-Skull', '.magic-dr-arm-l', '.magic-dr-arm-r'], { y: 0, rotation: 0 });
      }
      ...
      const startBlinking = () => {
        ...
        drBlinkTween.to('.dr-eye-close', { autoAlpha: 1, duration: 0.1 })
      ...
      gsap.to('.magic-mild-ice', { rotation: 360, duration: 20, repeat: -1, ease: 'linear', transformOrigin: 'center center' })
      gsap.to('.magic-mild-sunmoon', { rotation: -360, duration: 25, repeat: -1, ease: 'linear', transformOrigin: 'center center' })
      gsap.to('.magic-mild-fire', { rotation: 360, duration: 18, repeat: -1, ease: 'linear', transformOrigin: 'center center' })
      ...
      magicEntrance
        ...
        .fromTo(['.magic-mild-ice', '.magic-mild-sunmoon', '.magic-mild-fire'],
          { scale: 0, autoAlpha: 0 },
          { scale: 1, autoAlpha: 1, duration: 1.5, ease: 'back.out(1.2)', stagger: 0.2, transformOrigin: 'center center' }, "<")
        .fromTo('.magic-dr-Skull',
          { x: -100, y: -100, autoAlpha: 0, scale: 0.5, rotation: -45 },
          { x: 0, y: 0, autoAlpha: 1, scale: 1, rotation: 0, duration: 1, ease: 'power2.out' }, "-=1")
        .fromTo(['.magic-dr-arm-l', '.magic-dr-arm-r'],
          { y: 50, autoAlpha: 0 },
          { y: 0, autoAlpha: 1, duration: 1, stagger: 0.2, ease: 'power2.out' }, "-=0.5")

      // --- Mild Attack Loop ---
      const mildAttack = gsap.timeline();
      const magicElements = ['.magic-mild-ice', '.magic-mild-sunmoon', '.magic-mild-fire'];
      const targets = [{ x: -400, y: -500 }, { x: -800, y: -500 }, { x: -700, y: -100 }];

      for (let i = 0; i < 3; i++) {
        const cycleLabel = `cycle${i}`;
        mildAttack.addLabel(`${cycleLabel}_start`);
        magicElements.forEach((el, index) => {
          mildAttack.to(el, { x: targets[index].x, y: targets[index].y, duration: 0.8, ease: 'power2.in' }, `${cycleLabel}_start+=${index * 0.1}`);
        });
        mildAttack.call(playFlash);
        mildAttack.addLabel(`${cycleLabel}_return`);
        magicElements.forEach((el, index) => {
          mildAttack.to(el, { x: 0, y: 0, duration: 0.8, ease: 'power2.out' }, `${cycleLabel}_return+=${index * 0.1}`);
        });
      ...
      tl.set(bgRef.current, { scale: 2, xPercent: 25, yPercent: 25 })
        ...
        // ซ่อน Magic Effects
        .to(['.magic-dr-Skull', '.magic-dr-arm-l', '.magic-dr-arm-r', '.magic-dr-ciecle-l', '.magic-dr-ciecle-r'], {
          autoAlpha: 0, duration: 0.5
        })

        // Dr.Gamer ร่วง
        .to('.dr-gamer-container', {
          y: '25vh',
          rotation: -90,
          ease: 'bounce.out',
          duration: 2
        ...
        .to('.dr-eye-open', { autoAlpha: 0, duration: 0.05, overwrite: true }, "<")
        .to('.dr-eye-close', { autoAlpha: 1, duration: 0.05, overwrite: true }, "<")
        ...
        .fromTo('.narration-final-1',
          { autoAlpha: 0, scale: 0.8, rotation: 5 },
          { autoAlpha: 1, scale: 0.8, rotation: 1, duration: 1.5, ease: 'back.out(1.7)' },
          'announcement+=1' // เริ่ม 1 วิ หลังซูม
        )
        // ค้างกล่อง 1 ไว้
        .to('.narration-final-1', { autoAlpha: 0, duration: 1 }, 'announcement+=3.5')

        // 2. กล่อง 2A: ไม่น่าเชื่อว่า... มายด์
        .fromTo('.narration-final-2a',
          { autoAlpha: 0, scale: 0.8, rotation: -5 },
          { autoAlpha: 1, scale: 0.8, rotation: -1, duration: 1.5, ease: 'back.out(1.7)' },
          'announcement+=4.5' // เริ่ม 4.5 วิ หลังซูม (ต่อจากกล่อง 1)
        )
        // ค้างกล่อง 2A ไว้
        .to('.narration-final-2a', { autoAlpha: 0, duration: 1 }, 'announcement+=7')

        // 3. กล่อง 2B: น่าจับตามอง (ต่อเนื่อง)
        .fromTo('.narration-final-2b',
  ...
  return (
    <div ref={containerRef} className="relative h-screen w-full overflow-hidden bg-black">
      <div ref={bgRef} className="absolute inset-0 h-full w-full will-change-transform">
        ...
        {/* 2. Dr.Gamer */}
        <div className="dr-gamer-container absolute top-[25%] left-[0%] z-20 h-[500px] w-[320px] md:h-[750px] md:w-[500px]">
          <div className="relative h-full w-full">
            ...
            <div className="dr-head-group absolute left-0 top-[1%] z-30 w-full">
              {/* ตาเปิด (Base) */}
              <img src="/assets/part3/Character/Battle/Dr_Gamer/Pose/Dr_Gamer_head_op.png" className="dr-eye-open w-full object-contain" alt="Eye Open" />
        ...
        <div className="absolute top-[15%] left-[0%] z-20 h-[1300px] w-[1300px]">
          <div className="relative h-full w-full">
            {/* bone R */}
            <img src="/assets/part3/Character/Battle/Dr_Gamer/Magic/R_circle.png" className="magic-dr-ciecle-l absolute left-[27%] top-[12%] z-10 w-[200px]" alt="L circle" />
            <div className="absolute w-full left-[0%] bottom-[13%] h-full pointer-events-none">
            ...
            <img src="/assets/part3/Character/Battle/Dr_Gamer/Magic/R_circle.png" className="magic-dr-ciecle-r absolute left-[0%] bottom-[35%] z-10 w-[300px]" alt="R circle" />
            <div className="absolute left-[-5%] top-[3%] w-full h-full pointer-events-none">
        ...
        {/* 5. Mild [Magic] */}
        <div className="absolute top-[15%] right-[-7%] z-20 h-[1300px] w-[1300px] pointer-events-none">
          <div className="relative h-full w-full">
            <img src="/assets/part3/Character/Battle/Mild/Magic1/Fire_Circle.png" className="magic-mild-fire absolute right-[0%] top-[20%] z-10 w-[350px]" alt="Ice" />
            <img src="/assets/part3/Character/Battle/Mild/Magic1/Ice_Circle.png" className="magic-mild-ice absolute right-[27%] top-[38%] z-10 w-[300px]" alt="SunMoon" />
            <img src="/assets/part3/Character/Battle/Mild/Magic1/Sun&moon_Circle.png" className="magic-mild-sunmoon absolute right-[2%] bottom-[18%] z-10 w-[400px]" alt="Fire" />
          </div>
        </div>

        {/* 6. Flash Overlay */}
        <div className="white-flash absolute inset-0 z-50 bg-white pointer-events-none opacity-0 mix-blend-overlay"></div>
        ...
        <div className="narration-box narration-final-2a absolute left-1/2 top-1/2 z-40 max-w-xl -translate-x-1/2 -translate-y-1/2 rotate-[-2deg] rounded-lg border-8 border-pink-700 bg-white/90 p-8 text-center shadow-2xl shadow-pink-900/50 opacity-0 will-change-transform">
          <p className=" text-xl font-medium leading-relaxed text-black md:text-2xl">
            ...
            <span className="text-4xl text-pink-600 font-medium block mt-1">
              "มายด์"
            </span>
          </p>
          <p className=" text-xl font-medium leading-relaxed text-black mt-2 md:text-2xl">
        ...
        <div className="narration-box narration-final-2b absolute left-1/2 top-1/2 z-40 max-w-xl -translate-x-1/2 -translate-y-1/2 rotate-[3deg] rounded-lg border-8 border-green-700 bg-white/90 p-8 text-center shadow-2xl shadow-green-900/50 opacity-0 will-change-transform">
          <p className="mt-4 text-xl font-medium text-black leading-relaxed">
            ถือว่าน่าจับตามองกันเลยทีเดียว<br />
            ว่าเธอจะสามารถไปได้ไกล<br />
            แค่ไหนกันนะครับ!!
          </p>
        </div>
