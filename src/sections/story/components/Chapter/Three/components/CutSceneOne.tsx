...
const SmartSplitText = ({ children, className = '' }: { children: string, className?: string }) => {
      ...
      {words.map((word, wordIndex) => (
        <span key={wordIndex} className="inline-block whitespace-nowrap">
          {word.split('').map((char, charIndex) => (
            <span key={charIndex} className="magic-char inline-block will-change-transform">
              {char}
            </span>
          ))}
...
// ข้อมูลบทพูด
const DIALOGUES = [
  { id: 1, speaker: 'มายด์', side: 'right', text: 'นี่จะรีบไปไหน ฉันยังไม่ได้ขอบคุณฮันนี่เลยนะที่ทำให้ฉันได้พิสูจน์ตัวเองกับเพื่อนๆ และคนอื่นๆ ในโรงเรียน' },
  { id: 2, speaker: 'ฮันนี่', side: 'left', note: '(เวทตัวอักษร)', text: 'หน้าที่ของฉันหมดลงตรงนี้เธอไม่จำเป็นต้องมีฉันอยู่หรอก เธอได้แสดงความสามารถของตัวเองให้ทุกคนได้เห็นแล้ว' },
  { id: 3, speaker: 'ฮันนี่', side: 'left', note: '(เวทตัวอักษร)', text: 'แต่ว่า.. ก็ยินดีด้วยนะ ฉันภูมิใจที่ได้เป็นอาจารย์ให้เธอนะ ฉันดีใจที่เธอสามารถพิสูจน์ตัวเองได้ รางวัลนี่มันเหมาะกับเธอจริงๆ' },
  { id: 4, speaker: 'มายด์', side: 'right', text: 'ขอบคุณนะคะที่คอยเป็นกำลังใจให้ในยามที่ฉันกลุ้มใจ ขอบคุณนะที่มอบความกล้าให้ฉัน ฉันดีใจที่ได้รู้จักกับฮันนี่จริงๆ นะ' },
  { id: 5, speaker: 'ฮันนี่', side: 'left', note: '(เวทตัวอักษร)', text: 'ฉันเองก็ดีใจที่ได้รู้จักกับเธอนะมายด์' },
  { id: 6, speaker: 'ฮันนี่', side: 'left', note: '(เสียงเบาๆ)', text: 'สุดท้ายนี้...' },
  { id: 7, speaker: 'ฮันนี่', side: 'left', isFinal: true, text: 'ขอบคุณเธอที่มอบความกล้าให้ฉัน ขอบคุณนะมายด์' },
]
...
export default function CutSceneOne() {
  ...
  useLayoutEffect(() => {
    const ctx = gsap.context(() => {
      ...
      gsap.set(imageWrapperRef.current, { scale: 1.6, xPercent: -20, autoAlpha: 0, transformOrigin: 'center center' })
      ...
      DIALOGUES.forEach((item, index) => {
        ...
        if (isMagic) {
          tl.to(`.magic-text-${index} .magic-char`, {
            y: 0,
            autoAlpha: 1,
            scale: 1,
            duration: 0.5,
        ...
        if (index < DIALOGUES.length - 1) {
          ...
          tl.to(selector, { autoAlpha: 0, y: -20, duration: 1 }, 'hide-' + index)
        }
      })
  ...
  const getBoxStyle = (isRight: boolean, isMagic: boolean) => {
    let classes = "max-w-[90%] md:max-w-[75%] p-4 md:p-6 rounded-2xl backdrop-blur-sm shadow-xl border "
    if (isMagic) classes += "bg-transparent border-white/40 text-yellow-100 shadow-none"
  ...
  return (
    <div ref={containerRef} className="relative flex h-screen w-full items-center justify-center overflow-hidden bg-black font-sans">
      <div
        ref={imageWrapperRef}
        className="aspect-video max-h-screen w-full will-change-transform"
        style={{ backgroundImage: `url('${bgPath}')`, backgroundSize: 'cover', backgroundPosition: 'center', backgroundRepeat: 'no-repeat' }}
      />

      <div ref={textWrapperRef} className="absolute bottom-0 left-0 z-20 w-full h-[30vh] flex flex-col justify-end pb-10 px-4 md:px-20 bg-gradient-to-t from-black via-black/80 to-transparent pointer-events-none">
        <div className="relative w-full h-full max-w-5xl mx-auto">
          {DIALOGUES.map((item, index) => {
            ...
            return (
              <div key={item.id} className={`dialogue-${index} absolute bottom-0 w-full flex flex-col mb-4 opacity-0 will-change-transform ${isRight ? 'items-end text-right' : 'items-start text-left'}`}>
                <span className={`mb-2 font-bold text-xl md:text-2xl drop-shadow-md ${isRight ? 'text-pink-400' : 'text-yellow-400'}`}>{item.speaker}</span>
