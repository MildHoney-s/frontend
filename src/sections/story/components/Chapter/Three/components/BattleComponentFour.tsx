...
export default function BattleComponentFour() {
  ...
  useLayoutEffect(() => {
    const ctx = gsap.context(() => {
      ...
      gsap.set('.mild-magic-back, .mild-magic-front', { autoAlpha: 0, scale: 0.8 })
      gsap.set('.mild-panda', { scaleX: -0, scaleY: 0, autoAlpha: 0 })
      gsap.set('.debirun-circle-1, .debirun-circle-2, .debirun-circle-3', { scale: 0, opacity: 0, transformOrigin: 'center center' })
      gsap.set('.debirun-magic-1, .debirun-magic-2, .debirun-magic-3', { scale: 0.5, opacity: 0, transformOrigin: 'bottom center' })
      ...
      mildBattleIdle.to('.mild-pose-2', { y: 5, duration: 2.5 }, 0).to('.Mild-hair', { rotation: 2, transformOrigin: 'top center', duration: 3 }, 0).to('.Mild-arm-l', { rotation: -3, transformOrigin: 'top right', duration: 3.5 }, 0)
      ...
      gsap.to('.debirun-magic-1, .debirun-magic-2, .debirun-magic-3', { y: -10, duration: 2, yoyo: true, repeat: -1, ease: 'sine.inOut', stagger: 0.2 })
      gsap.to('.mild-magic-float-group', { y: -15, duration: 2, yoyo: true, repeat: -1, ease: 'sine.inOut' })
      ...
      tl.fromTo('.intro-box-3',
        { autoAlpha: 0, scale: 2, rotate: -5 },
        { autoAlpha: 1, scale: 1, rotate: 0, duration: 1, ease: 'elastic.out(1, 0.5)' }
      )
        .to({}, { duration: 0.5 }) // แป๊บเดียวพอ กระตุ้น
        .to('.intro-box-3', { autoAlpha: 0, scale: 5, opacity: 0, duration: 1.5, ease: 'power2.in' }) // ขยายหายไปใส่คนดู


      // =========================================
      // Phase 1: Battle Begins (BG Move)
      // =========================================
      tl.set(bgRef.current, { scale: 2, xPercent: -50, yPercent: 10, transformOrigin: 'center center' })
        .to(bgRef.current, { xPercent: 40, yPercent: -40, ease: 'none', duration: 1 })
        .to(bgRef.current, { scale: 1, xPercent: 0, yPercent: 0, ease: 'power1.inOut', duration: 1 })

      // ... (Phase 2-7 เหมือนเดิมเป๊ะ ก๊อปปี้วางต่อจากนี้ได้เลยครับ) ...
      // Phase 2: Debirun Magic
      tl.to('.debirun-circle-1, .debirun-circle-2, .debirun-circle-3', { scale: 1, opacity: 1, duration: 0.8, stagger: 0.2, ease: 'back.out(1.7)' })
        .to('.debirun-magic-1, .debirun-magic-3, .debirun-magic-2', { scale: 1, opacity: 1, duration: 1, stagger: 0.1, ease: 'power2.out' }, '<0.3')
      // Phase 3: Mild Transform
      tl.to('.mild-pose-1', { autoAlpha: 0, duration: 0.5, scale: 0.9 }).fromTo('.mild-pose-2', { autoAlpha: 0, scale: 1.1 }, { autoAlpha: 1, scale: 1, duration: 0.5 }, '<')
      // Phase 4: Mild Magic
      tl.to('.mild-magic-back, .mild-magic-front', { autoAlpha: 1, scale: 1, duration: 1, ease: 'power2.out' })
      // Phase 5: Panda Attack
      tl.to('.mild-panda', { scaleX: -1, scaleY: 1, autoAlpha: 1, duration: 0.5, stagger: 0.1, ease: 'back.out(1.5)' })
      tl.to('.mild-panda', { x: 600, y: -400, scaleX: -1.5, scaleY: 1.5, opacity: 0, duration: 1.2, stagger: 0.1, ease: 'power2.in' })
      tl.to('.white-flash', { opacity: 0.8, duration: 0.1, yoyo: true, repeat: 3 }, '-=0.5')
      // Phase 6: Debirun Defeat
      tl.to('.debirun-magic-group2', { opacity: 0, scale: 0.8, duration: 1, ease: 'power2.in' }, '<')
      tl.to('.debirun-container', { rotation: -90, yPercent: 20, xPercent: 10, opacity: 0.5, filter: 'grayscale(100%)', ease: 'bounce.out', duration: 2, onComplete: () => { if (debirunBlinkTween) debirunBlinkTween.pause(); gsap.set('.debirun-eye-open', { autoAlpha: 0 }); gsap.set('.debirun-eye-close', { autoAlpha: 1 }); }, onReverseComplete: () => { if (debirunBlinkTween) debirunBlinkTween.resume(); gsap.set('.debirun-eye-open', { autoAlpha: 1 }); gsap.set('.debirun-eye-close', { autoAlpha: 0 }); } }, '<')
      // Phase 7: Victory
      tl.to(bgRef.current, { scale: 1.8, xPercent: 40, yPercent: -40, duration: 2.5, ease: 'power2.inOut' })
      tl.fromTo('.narration-box-1', { autoAlpha: 0, scale: 0, rotation: -10 }, { autoAlpha: 1, scale: 1, rotation: 0, duration: 1, ease: 'elastic.out(1, 0.6)' }, '-=1.0')
      tl.to({}, { duration: 0.5 })
      tl.to('.narration-box-1', { autoAlpha: 0, scale: 1.2, filter: 'blur(10px)', duration: 0.5 })
      tl.fromTo('.narration-box-2', { autoAlpha: 0, y: 50 }, { autoAlpha: 1, y: 0, duration: 0.8, ease: 'back.out(1.5)' })
  ...
  return (
    <div ref={containerRef} className="relative h-screen w-full overflow-hidden bg-black">
      ...
      <div ref={bgRef} className="absolute inset-0 h-full w-full bg-cover bg-center bg-no-repeat will-change-transform origin-center" style={{ backgroundImage: `url('${bgPath}')` }}>
        ...
        {/* 4. MAGIC DEBIRUN ... */}
        <div className="debirun-magic-group2 absolute bottom-[0%] right-[2%] z-10 w-full h-full pointer-events-none">
          <img src="/assets/part3/Character/Battle/Mild/Magic1/Sun&moon_Circle.png" className="debirun-circle-1 absolute right-[18%] bottom-[40%] z-10 w-[200px]" alt="circle1" />
          <img src="/assets/part3/Character/Battle/Mild/Magic1/Sun&moon_Circle.png" className="debirun-circle-3 absolute right-[12%] bottom-[25%] z-10 w-[200px]" alt="circle2" />
          <img src="/assets/part3/Character/Battle/Mild/Magic1/Sun&moon_Circle.png" className="debirun-circle-2 absolute right-[-3%] bottom-[23%] z-10 w-[200px]" alt="circle3" />
          <img src="/assets/part3/Character/Battle/debirun/Magic/magic_1.png" className="debirun-magic-1 absolute right-[10%] bottom-[26%] z-10 w-[750px]" alt="magic-main-1" />
          <img src="/assets/part3/Character/Battle/debirun/Magic/magic_3.png" className="debirun-magic-3 absolute right-[7%] bottom-[8%] z-10 w-[600px]" alt="magic-main-3" />
          <img src="/assets/part3/Character/Battle/debirun/Magic/magic_2.png" className="debirun-magic-2 absolute right-[2%] bottom-[10%] z-10 w-[600px]" alt="magic-main-2" />
        </div>
        {/* 3. MILD POSE 1 & 2 ... */}
        ...
        <div className="mild-pose-2 absolute bottom-[-20%] left-[-3%] z-20 h-[500px] w-[320px] md:h-[750px] md:w-[500px] opacity-0">
        ...
        {/* 5. MAGIC MILD ... */}
        <div className="mild-magic-back absolute bottom-[-10%] left-[-12%] z-10 opacity-0 pointer-events-none">
          <div className="mild-magic-float-group relative w-[700px] h-[700px]">
        ...
        <div className="mild-magic-front absolute bottom-[-10%] left-[-12%] z-30 opacity-0 pointer-events-none">
          <div className="mild-magic-float-group relative w-[700px] h-[700px]">
          ...
          <img src="/assets/part3/Character/Battle/Mild/Magic2/panda.png" className="mild-panda absolute left-[10%] top-[40%] z-50 w-[150px] opacity-0" alt="panda-1" />
          <img src="/assets/part3/Character/Battle/Mild/Magic2/panda.png" className="mild-panda absolute left-[30%] top-[30%] z-50 w-[170px] opacity-0" alt="panda-2" />
          <img src="/assets/part3/Character/Battle/Mild/Magic2/panda.png" className="mild-panda absolute left-[40%] bottom-[30%] z-50 w-[100px] opacity-0" alt="panda-3" />
        </div>
      ...
      {/* กล่อง 1: จบกันไปแล้ว */}
      <div className="intro-box-1 narration-final-1 absolute left-1/2 top-[25%] z-40 max-w-xl -translate-x-1/2 -translate-y-1/2 rotate-1 rounded-lg border-8 border-yellow-500 bg-white/90 p-8 text-center shadow-2xl shadow-yellow-800/50 opacity-0 will-change-transform">
        <p className=" text-2xl font-medium leading-tight text-black md:text-3xl">
          ไม่น่าเชื่อว่าคู่ชิงจะเป็น
        </p>
        <p className=" text-xl font-medium leading-relaxed text-red-700 mt-2 md:text-2xl">
          <i><b> MILD-R </b></i> พบกับ <i><b> อดีตแชมป์ Debby</b></i>
        </p>
      </div>

      {/* กล่อง 2B: น่าจับตามอง (ส่วนขยาย) */}
      <div className="intro-box-2 narration-final-2b absolute left-1/2 top-[25%] z-40 max-w-xl -translate-x-1/2 -translate-y-1/2 rotate-[3deg] rounded-lg border-8 border-black bg-white/90 p-8 text-center shadow-2xl shadow-green-900/50 opacity-0 will-change-transform">
        <p className="mt-4 text-xl font-medium text-black leading-relaxed">
          ถือว่าเป็นครั้งแรกของเธอเลย <br />ที่ได้ลงแข่งรายการนี้แต่ก็มาได้ถึงรอบชิง
        </p>
      </div>


      {/* กล่อง 2B: น่าจับตามอง (ส่วนขยาย) */}
      <div className="intro-box-3 narration-final-2b absolute left-1/2 top-[25%] z-40 max-w-xl -translate-x-1/2 -translate-y-1/2 rotate-[3deg] rounded-lg border-8 border-black bg-white/90 p-8 text-center shadow-2xl shadow-green-900/50 opacity-0 will-change-transform">
        <p className="mt-4 text-xl font-medium text-black leading-relaxed">
        ...
        <div className="relative overflow-hidden rounded-2xl border-4 border-yellow-400 bg-gradient-to-b from-white to-yellow-50 p-1 shadow-[0_0_60px_rgba(234,179,8,0.6)] backdrop-blur-sm">
          <div className="rounded-xl border-2 border-dashed border-yellow-200 bg-white/95 px-8 py-10 text-center shadow-inner">
            ...
            <div className="mt-6 transform transition-all duration-500 hover:scale-105">
              <span className="block text-7xl font-black italic tracking-tighter text-transparent bg-clip-text bg-gradient-to-r from-pink-500 via-red-500 to-pink-500 drop-shadow-[0_5px_5px_rgba(0,0,0,0.2)] md:text-8xl">
                Mild-R!
              </span>
      ...
      <div className="narration-box-2 absolute left-1/2 top-[25%] z-40 w-[90%] max-w-xl -translate-x-1/2 -translate-y-1/2 opacity-0">
        <div className="relative rounded-xl border-[6px] border-double border-yellow-500 bg-white/95 p-8 text-center shadow-2xl shadow-yellow-600/40 transform rotate-1">
          <div className="mb-4 flex justify-center text-yellow-500 text-4xl">👑</div>
