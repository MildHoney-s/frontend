...
import { Member, ROLES, Role, TEAM } from './_mock/Member'
import { GalleryModal } from './components'

gsap.registerPlugin(ScrollTrigger)

function groupRolesByROLESOrder(list: Member[]): Array<[Role, Member[]]> {
  ...
  return ROLES.filter((r) => map.has(r)).map(
    (r) => [r, map.get(r)!] as [Role, Member[]],
  )
}

function getBioForRole(m: Member, role: Role): string {
  return m.roleBios?.[role] ?? m.bio ?? ''
}
...
export default function CreditsViewPage() {
  ...
  const groups = useMemo(() => {
    const raw = groupRolesByROLESOrder(TEAM)
    ...
    return raw.map(([role, members]) => {
      ...
      return [role, sorted] as [Role, Member[]]
    })
  }, [])
  ...
  return (
    <>
      <main
        ref={containerRef}
        className="min-h-screen px-6 py-10 font-prompt text-white"
        style={{
        ...
        <div className="mx-auto max-w-[1150px] rounded-xl bg-[#f5f5fa] px-6 py-10">
          <section className="credits-hero mx-auto mb-10 max-w-4xl text-center">
            ...
            <p className="mt-3 text-sm text-slate-500 sm:text-base">
              ขอบคุณทีมงานทุกคนที่ร่วมกันสร้างโปรเจกต์นี้ขึ้นมา
              ด้วยความตั้งใจและความทุ่มเทอย่างสุดหัวใจ
            </p>
          </section>
